...
class BaseAgent(torch.nn.Module):
    ...
    def __init__(self, config, env, device):
        ...
        #self._debug_log = open(config["debug_log"], "w")
    ...
    def load(self, in_file):
        ...
        #     print(value.shape)
        #     cond = "weight" in key
        #     if cond and value.shape[1] == old_input_dim:
        #         # Adjust weights: Add new input dimensions with random initialization
        #         new_weights = torch.empty(value.shape[0], new_input_dim)
        #         new_weights[:, :old_input_dim] = value
        #         new_weights[:, old_input_dim:] = torch.zeros(value.shape[0], new_input_dim - old_input_dim)
        #         adjusted_state_dict[key] = new_weights
        #     elif ("mean" in key or "std" in key) and value.shape[0] == old_input_dim:
        #         new_weights = torch.empty(new_input_dim)
        #         new_weights[:old_input_dim] = value
        #         new_weights[old_input_dim:] = torch.zeros(new_input_dim-old_input_dim)
        #         adjusted_state_dict[key] = new_weights
        #     else:
        #         # Keep other parameters (like biases) unchanged
    ...
    def _load_params(self, config):
        ...
        self._iters_per_checkpoint = config["iters_per_checkpoint"]
    ...
    def _build_logger(self, logger_type, log_file, config):
        if (logger_type == "tb"):
            ...
            log = wandb_logger.WandbLogger("rl_forge", config, connect_to_wandb=True, param_config=config)
        else:
    ...
    def _train_iter(self):
        self._init_iter() # clears exp_buffer in ppo_agent
